    ...
    int m) 
{
    ...
    size_t i;
    for (i = std::max(time_min_idx, 1); i < std::min(input.size()-1, (size_t)time_max_idx); i += m) 
    {
...
void MainWindow(ImGuiIO& io)
{
    ...
    double samples_in_range = (x_range.Max-x_range.Min)/(GetData()->time[1]-GetData()->time[0]);
    double max_samples_in_view = 10e3;
    double decimation_factor = std::max(samples_in_range/max_samples_in_view, 1.0);
    auto visible_min_it = std::lower_bound(GetData()->time.begin(), GetData()->time.end(), x_range.Min);
    int visible_min_idx = std::distance(GetData()->time.begin(), visible_min_it);
    visible_min_idx = std::max(visible_min_idx-1, 0);
    auto visible_max_it = std::upper_bound(GetData()->time.begin(), GetData()->time.end(), x_range.Max);
    int visible_max_idx = std::distance(GetData()->time.begin(), visible_max_it);
    //ImGui::Text("X Min: %.2f, X Max: %.2f, minIdx: %d, maxIdx: %d, decimationFac: %.2f", x_range.Min, x_range.Max, visible_min_idx, visible_max_idx, decimation_factor);

    ImPlotSubplotFlags const subplot_flags = ImPlotSubplotFlags_LinkAllX;
    uint8_t const len = subplots_map.size();
    ImPlot::BeginSubplots("",len,1, ImVec2(io.DisplaySize.x-value_column_size, io.DisplaySize.y-35), subplot_flags);
    ...
    for (int i = 0; i < len; ++i) 
    {
        ...
        if (i == 0)
        {
            ImVec2 label_pos = ImPlot::PlotToPixels(ImVec2(v_line_1_pos, 0)); // y doesn't matter for label
            label_pos.y = plot_y_pos[0] + 15 + 10.0f; // offset label above the line
            label_pos.x += 5.0f;
            ImGui::GetWindowDrawList()->AddText(label_pos, IM_COL32_WHITE, "1");
            label_pos = ImPlot::PlotToPixels(ImVec2(v_line_2_pos, 0)); // y doesn't matter for label
            label_pos.y = plot_y_pos[0] + 15 + 10.0f; // offset label above the line
            label_pos.x += 5.0f;
            ImGui::GetWindowDrawList()->AddText(label_pos, IM_COL32_WHITE, "2");
        }
                
        //for (auto signal_nameIt = subplots[i].begin(); signal_nameIt != subplots[i].end(); signal_nameIt++)
        for (auto& [signal_name, is_enabled] : subplots_map[i])
        {
            if (is_enabled)
            {
                //ImPlot::PlotStairs((*signal_nameIt).c_str(), time.data(), signals[*signal_nameIt].data(), samples);
                //std::vector<double> time_dec = Decimate(timeVec, timeVec, x_range.Min, x_range.Max, (int)(decimation_factor+0.5f));
                //std::vector<double> val     = Decimate(signals[signal_name], timeVec, x_range.Min, x_range.Max, (int)(decimation_factor+0.5f));
                std::vector<double> time_dec = Decimate(GetData()->time, visible_min_idx, visible_max_idx, (int)(decimation_factor+0.5f));
                std::vector<double> val     = Decimate(GetData()->signals[signal_name], visible_min_idx, visible_max_idx, (int)(decimation_factor+0.5f));
    ...
    auto vline_1_it = std::upper_bound(GetData()->time.begin(), GetData()->time.end(), v_line_1_pos);
    if (vline_1_it != GetData()->time.begin()) --vline_1_it;
    int vline_1_idx = std::distance(GetData()->time.begin(), vline_1_it);

    auto vline_2_it = std::upper_bound(GetData()->time.begin(), GetData()->time.end(), v_line_2_pos);
    ...
    for (int i = 0; (size_t)i < subplots_map.size(); ++i) 
    {
        ...
        for (auto& [signal_name, is_enabled] : subplots_map_i)
        {
            ...
            {
                ImGui::TableNextRow();
                ImGui::TableSetColumnIndex(0); ImGui::Text("%s", signal_name.c_str());
                ImGui::TableSetColumnIndex(1); ImGui::Text("%.4f", GetData()->signals[signal_name][vline_1_idx]);
                ImGui::TableSetColumnIndex(2); ImGui::Text("%.4f", GetData()->signals[signal_name][vline_2_idx]);
            }
        }
